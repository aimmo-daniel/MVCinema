...
<script>
	...
	function selectMovie(tagid) {
		...
		var param="idx="+movie_idx;
		$.ajax({
			type:"post",
			url:"${path}/ticket/select_movie.do",
			data:param,
	...
	function selectDay(y, m, d) {
		...
		$.ajax({
			type : "get",
			url : "${path}/ticket/screen_time.do?movie_idx=" + movie_idx
	...
	function selectTime(timeid) {
		...
		$.ajax({
			type:"get",
			url:"${path}/ticket/select_time.do?screen_idx="+start_time,
			success:function(result){
...
<%@include file="../../sangjin/home/loginbar.jsp"%>
	<div align="left" style="margin-left: 70%">
		<a href="${path}/ticket/movie_ticket_page.do" role="button"
			class="btn btn-primary">영화별 예매</a> <a href="${path}/ticket/date_ticket_page.do" role="button"
					...
					<ul class="list-group" style="overflow: auto; height: 400px;">
						<c:forEach var="row" items="${map.list}">
							<li class="list-group-item" onclick="selectMovie('${row.movie_idx}')"
								...
								<b>${row.title}</b> <input type="hidden" id="release_date"
			...
			<div class="col-md-4" id="ticket_time">
				...
				<ul class="list-group" style="overflow: auto; height: 400px;">
					<c:forEach var="i" begin="1" end="8">
						<li class="list-group-item">&nbsp;</li>
						<c:if test="${i == 2 }">
							<li class="list-group-item" style="text-align: center;"><b>영화 와
									시간을 선택해주세요</b></li>
						</c:if>
					</c:forEach>
				</ul>
